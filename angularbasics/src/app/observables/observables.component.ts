  ...
  data : any[] = [];
    ...
    getData1() {
      this.observable.subscribe({
        ...
        complete: () => {alert ('Data retrived successfully')}
       })
    }

    getData2() {   // using of method from rxjs which returns observable 
      // you can pass as many data as you want 
      let observable = of(4,9,"A","B",12)   // it returns observable with this data wraps around it 
      observable.subscribe({ // this only has two signals next and complete 
    ...
    getData4() {
      ...
      let subscription_obj = interval(1000)
      .subscribe({  // to subscribe the interval 
        next:(v) => this.data.push(v)
      })
